...
async def test_mettre_a_jour_un_livre():
    ...
    nouvel_auteur = create_utilisateur()
    genre = create_genre()
    ...
    creer_auteur = CreerUtilisateur(repo_utilisateur)
    ...
    creer_utilisateur = await creer_auteur.executer(auteur, auteur.mot_de_passe)
    creer_deuxieme_auteur = await creer_auteur.executer(nouvel_auteur, nouvel_auteur.mot_de_passe)
    creer_genre = await creer_un_genre.executer(genre)

    utilisateur = await recuperer_utilisateur.executer(creer_utilisateur.id)
    deuxieme_auteur = await recuperer_utilisateur.executer(creer_deuxieme_auteur.id)
    ...
    livre_modifier = LivreUpdateSchema(
        auteur= deuxieme_auteur,
        auteur_id=deuxieme_auteur.id
        )
    ...
    assert livre_recuperer_apres_update.auteur_id == deuxieme_auteur.id