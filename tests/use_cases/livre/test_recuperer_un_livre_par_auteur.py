...
async def test_recuperer_un_livre_par_auteur():
    ...
    recuperer_un_livre_par_auteur = RecupererLivreParAuteur(repo_livre)
    ...
    livre_recuperer: list[Livre] = await recuperer_un_livre_par_auteur.executer(nouveau_livre.auteur)

    assert len(livre_recuperer) == 1
    assert livre_recuperer[0].id == nouveau_livre.id
    assert livre_recuperer[0].auteur.id == utilisateur.id
