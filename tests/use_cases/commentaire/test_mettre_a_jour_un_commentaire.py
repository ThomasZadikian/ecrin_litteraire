...
from src.domain.model.commentaire import Commentaire, CommentaireCreationSchema, CommentaireUpdateSchema
...
from src.use_cases.commentaire.modifier_un_commentaire import ModifierUnCommentaire
...
async def test_creer_et_recuperer_un_commentaire(): 
    ...
    modifier_un_commentaire = ModifierUnCommentaire(repo_commentaire)
    ...
    commentaire_modifier = CommentaireUpdateSchema(
        contenu = "Nouveau contenu"
    )

    await modifier_un_commentaire.executer(commentaire_recuperer.id, commentaire_modifier)

    commentaire_final = await recuperer_un_commentaire.executer(commentaire_recuperer.id)

    assert commentaire_final.contenu == "Nouveau contenu"
